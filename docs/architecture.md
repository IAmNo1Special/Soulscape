...
- **Win32 mechanics**: frameless `WS_POPUP` topmost GL window sized to primary monitor; `WS_EX_LAYERED|TRANSPARENT|NOACTIVATE|TOOLWINDOW`; per-pixel alpha via `DwmEnableBlurBehindWindow` empty region; hit-testing = 100 ms cursor poll, point-in-ellipse per soul toggles click-through; focus never stolen; per-monitor-v2 DPI; no fullscreen parking — the overlay stays visible and topmost over every window, including exclusive-fullscreen apps.
